...
#[tokio::test]
async fn operator_recovery_end_to_end_requires_reconnect_safe_reopen_reapproval_guidance_on_same_selection_identity()
 {
    ...
    let preview = routing_service
        .preview_intent_request("req-operator-recovery-1")
        .await
    ...
    let first_eval = call_tool_json(
        ...
        json_map([
            ...
            (
                "request_id",
                Value::String("req-operator-recovery-1".to_owned()),
            ),
        ]),
    )
    .await;
    ...
    reservations
        .hold(ReservationRequest {
            reservation_id: "reservation-operator-recovery-1".to_owned(),
            execution_id: "req-operator-recovery-1".to_owned(),
            asset: "USDC".to_owned(),
            amount: 3_000,
        })
        .await
    ...
    let approved = call_tool_json(
        ...
        json_map([
            ...
            (
                ...
                json!({
                    "approved_by": "owner",
                    "note": "approve before assembled reconnect/recovery coverage"
                }),
            ),
        ]),
    )
    ...
    let monitoring_before_override = read_resource_json(&client, monitoring_uri.clone()).await;
    let mut gaps = Vec::new();
    if monitoring_before_override["strategy_id"] != "strategy-lp-1" {
        gaps.push("assembled flow must project a real strategy_id before reconnect".to_owned());
    }
        ...
        json_map([
            ...
            (
                ...
                json!({
                    ...
                    "rationale": "change the readiness-sensitive budget after runtime activation"
                }),
            ),
        ]),
    )
    ...
    if overridden["selection_id"] != selection_id {
        gaps.push(
            "assembled post-approval override must keep the same selection identity stable"
                .to_owned(),
        );
    }
    ...
    {
        gaps.push(format!(
            "monitoring resource must reject reads before onboarding.bootstrap_install reopens the install, got {pre_reopen_monitoring_error}"
        ));
    }
    ...
    if reopened_install["claim_disposition"] != "reopened" {
        gaps.push("bootstrap reopen must preserve the same install identity on the assembled recovery path".to_owned());
    }

    let monitoring_after_reconnect =
        read_resource_json(&reconnected_client, monitoring_uri.clone()).await;
    if monitoring_after_reconnect["strategy_id"] != "strategy-lp-1" {
        gaps.push(
            "assembled reconnect path must preserve refreshed strategy_id after bootstrap reopen"
                .to_owned(),
        );
    }
    if monitoring_after_reconnect["hold_reason"] != "approved_selection_revision_stale" {
        gaps.push("assembled reconnect path must preserve approved_selection_revision_stale after reopen-worthy override".to_owned());
    }
    ...
    match call_tool_json_result(
        ...
        json_map([
            ...
            (
                "reason",
                Value::String("operator requested reopen after reconnect".to_owned()),
            ),
        ]),
    )
    .await
    {
        Ok(result) => {
            if result["selection_id"] != selection_id {
                gaps.push("strategy_selection.reopen must reopen the same selection identity instead of minting a new one".to_owned());
            }
            if result["status"] != "reopened" {
                gaps.push("strategy_selection.reopen must return status=reopened on the assembled path".to_owned());
            }
            if result["approval_history_uri"] != approval_history_uri {
                gaps.push("strategy_selection.reopen must return approval_history_uri for reconnect-safe rereads".to_owned());
            }
            if result["diff_uri"] != diff_uri {
                gaps.push("strategy_selection.reopen must return diff_uri for reconnect-safe rereads".to_owned());
            }
        }
        Err(error) => gaps.push(format!(
            "assembled recovery path must support strategy_selection.reopen on the shipped MCP surface, got {error}"
        )),
    }

    match read_resource_json_result(&reconnected_client, approval_history_uri.clone()).await {
        Ok(history) => {
            if !history["events"]
                .as_array()
                .is_some_and(|items| items.iter().any(|item| item["event_kind"] == "approved"))
            {
                gaps.push("assembled recovery path must keep durable approval history after reconnect".to_owned());
            }
            if !history["events"]
                .as_array()
                .is_some_and(|items| items.iter().any(|item| item["event_kind"] == "reopened"))
            {
                gaps.push("assembled recovery path must expose a reopened event on the same selection identity after reconnect".to_owned());
            }
        }
        Err(error) => gaps.push(format!(
            "assembled recovery path must expose approval-history rereads after reconnect, got {error}"
        )),
    }

    match read_resource_json_result(&reconnected_client, diff_uri.clone()).await {
        Ok(diff) => {
            if diff["selection_id"] != selection_id {
                gaps.push(
                    "assembled diff reread must retain the same selection identity after reconnect"
                        .to_owned(),
                );
            }
            if diff["approval_stale_reason"] != "readiness_sensitive_override" {
                gaps.push(
                    "assembled diff reread must explain why reapproval is required".to_owned(),
                );
            }
        }
        Err(error) => gaps.push(format!(
            "assembled recovery path must expose diff rereads after reconnect, got {error}"
        )),
    }
    ...
    assert!(
        gaps.is_empty(),
        "S03 assembled operator-recovery contract missing reconnect-safe reopen, approval history, diff inspection, or recovery guidance: {}",
        gaps.join("; ")
    );
}
...
fn strategy_request() -> JsonRpcRequest<serde_json::Value> {
    JsonRpcRequest::new(
        "req-strategy-operator-recovery",
        "daemon.registerStrategy",
        json!({
            "request_id": "req-strategy-operator-recovery",
...
fn intent_request() -> JsonRpcRequest<serde_json::Value> {
    JsonRpcRequest::new(
        "req-operator-recovery-1",
        "daemon.submitIntent",
        json!({
            "request_id": "req-operator-recovery-1",
            "request_kind": "intent",
            "source_agent_id": "agent-main",
            "submitted_at": "2026-03-12T00:00:00Z",
            "payload": {
                "intent_id": "intent-operator-recovery-1",
